 ...
 */
abstract contract VRFConsumerBase is Modifiers, VRFRequestIDBase {
    /**
   ...
   */
    function requestRandomness(bytes32 _keyHash, uint256 _fee) internal returns (bytes32 requestId) {
        s.LINK.transferAndCall(s.vrfCoordinator, _fee, abi.encode(_keyHash, USER_SEED_PLACEHOLDER));
    ...
    */
    function setVrfCoordinator(address vrfCoordinator_) external onlyOwner {
        s.vrfCoordinator = vrfCoordinator_;
    }

    /**
    * @param link_ address of LINK token contract
    */
    function setLinkAddress(address link_) external onlyOwner {
        s.LINK = LinkTokenInterface(link_);
    }
